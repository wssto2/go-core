...
func (d *Datatable[T]) WithoutDeleted(column string) *Datatable[T] {
	d.db = d.db.Where(fmt.Sprintf("%s.%s IS NULL", d.tableName, database.QuoteColumn(column)))
	return d
}
			...
			for _, field := range d.searchableFields {
				if after, ok := strings.CutPrefix(field, "concat:"); ok {
					...
					for _, column := range cols {
						concatExpr = append(concatExpr, fmt.Sprintf("IFNULL(%s.%s, '')", d.tableName, database.QuoteColumn(column)))
					}
					wordConditions = append(wordConditions, "CONCAT("+strings.Join(concatExpr, ", ' ', ")+") LIKE ?")
				} else {
					wordConditions = append(wordConditions, fmt.Sprintf("%s.%s LIKE ?", d.tableName, database.QuoteColumn(field)))
				}
